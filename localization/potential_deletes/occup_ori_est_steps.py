...
def occup_ori_est(pi_data, cfg):
  ...
  ''' Track 2D pose from each Pi data '''
  pi_pid_seq = {}
  for pi in pi_data:
    pi_pid_seq[pi] = {}
  ...
  ''' Collect kps track into np.array '''
  kps_seq_pi_pid_np = {}
  ...
  if cfg.viz_pose3d_calib_match_2d:
    for pi in pose3d_pi_pid:
      for pid in pose3d_pi_pid[pi]:
        ...
        kps_seq = kps_seq_pi_pid_np[pi][pid]
        dts = pi_pid_seq[pi][pid]['dt']
        viz_pose3d_calib_match_2d(pose3d_calib, kps_seq, skeleton, dts, pi, pid, cfg)
  ...
  if cfg.viz_chest_3d:
    for pi in pose3d_pi_pid:
      for pid in pose3d_pi_pid[pi]:
        ...
        dts = pi_pid_seq[pi][pid]['dt']
        viz_chest_3d(pose3d_calib, chest_norm_3d, skeleton, dts, pi, 'chest_3d', cfg)
    ...
    if cfg.viz_chest_3d_rot:
      for pi in pose3d_pi_pid:
        for pid in pose3d_pi_pid[pi]:
          ...
          dts = pi_pid_seq[pi][pid]['dt']
          viz_chest_3d(pose3d_calib, chest_norm_3d_rot, skeleton, dts, pi, 'chest_3d_rot', cfg)
  ...
  if cfg.viz_occup_ori_ep6_pi:
    for pi in EP6_feet_pos_pi_pid:
      for pid in EP6_feet_pos_pi_pid[pi]:
        ...
        ori_ep6 = ori_ep6_pi_pid[pi][pid]
        dts = pi_pid_seq[pi][pid]['dt']
        viz_occup_ori_ep6_pi(ori_ep6, EP6_feet_pos, ep6_map, dts, pi, pid, cfg)
