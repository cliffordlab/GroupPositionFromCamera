...
track_colors = cmap(np.linspace(0, 1, 23))*255
  ...
  if (len(centers) > 0):
    ...
    for j in range(len(tracker.tracks)):
      if (len(tracker.tracks[j].trace) > 1):
        occup_x = int(tracker.tracks[j].trace[-1][0,0])
        occup_y = int(tracker.tracks[j].trace[-1][0,1])
        ori_xy = tracker.tracks[j].trace[-1][0,2:]
        # print (ori_xy)
        # print (ori_xy.shape)
        # assert False
        ...
        cv2.putText(frame,str(tracker.tracks[j].trackId), (occup_x-10,occup_y-20), 0, 0.5, track_colors[j],2)
        ...
        if 1:
          for k in range(len(tracker.tracks[j].trace)):
            occup_x = int(tracker.tracks[j].trace[k][0,0])
            occup_y = int(tracker.tracks[j].trace[k][0,1])
            cv2.circle(frame,(occup_x, occup_y), 3, track_colors[j],-1)

      # detection
      if 1:
        centers = data[i]
        if centers.shape[0] > j:
          occup_x, occup_y = centers[j,:2]
          ori_x, ori_y = centers[j,2:] 
          facing_x, facing_y = int(occup_x + ori_x*10), int(occup_y + ori_y*10)
