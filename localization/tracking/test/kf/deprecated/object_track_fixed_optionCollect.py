'''
Collect state as not default but optional
'''
...
# from tracker_filterpy import Tracker
# from tracker_filterpy_correct_predupdate import Tracker
# from tracker_filterpy_correct_predupdate_fix import Tracker
from tracker_filterpy_correct_predupdate_fix_optionCollect import Tracker
# from tracker_NickNair import Tracker
#--------------
	...
	for i in range(data.shape[1]):
		...
		if (len(centers) > 0):
			...
			for trackId in tracker.tracks:
				...
				x = int(tracker.tracks[trackId].pos_state[0,0])
				y = int(tracker.tracks[trackId].pos_state[0,1])
				tl = (x-10,y-10)
				br = (x+10,y+10)
				cv2.rectangle(frame,tl,br,track_colors[j],1)
				cv2.circle(frame,(x,y), 6, track_colors[j],-1)
				cv2.putText(frame,str(tracker.tracks[trackId].trackId), (x-10,y-20),0, 0.5, track_colors[j],2)

				if 1:
					for k in range(len(tracker.tracks[trackId].trace)-1):
						x = int(tracker.tracks[trackId].trace[k][0,0])
						y = int(tracker.tracks[trackId].trace[k][0,1])
